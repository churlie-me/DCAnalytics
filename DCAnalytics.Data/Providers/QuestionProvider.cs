...
namespace DCAnalytics.Data
{
    public class QuestionProvider : Provider
    {
        ...
        internal Questions GetQuestions(Section section, string response_id = null)
        {
            ...
            {
                Questions _questions = new Questions();
                string query = $"select * from dsto_questions where [yref_section] ='{section.Key}' and Deleted=0 order by oid asc";
                var table = DbInfo.ExecuteSelectQuery(query);
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        InitQuestion(row, question, response_id);
                    }
                    return _questions;
                }
                return null;
        ...
        internal Questions GetQuestions(SubSection subSection, string response_id = null, int occurance = 0)
        {
            ...
            {
                Questions _questions = new Questions();
                string query = $"select * from dsto_questions where [yref_subsection]='{subSection.Key}' and Deleted=0 order by oid asc";
                var table = DbInfo.ExecuteSelectQuery(query);
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
