...
namespace DCAnalytics.Data
{
    public class UserProvider : Provider
    {
        ...
        public Users ClientUsers(int clientId)
        {
            ...
            try
            {
                string query = $"select * from dsto_user where client_id = '{clientId}' and Deleted=0";
                var table = DbInfo.ExecuteSelectQuery(query);
        ...
        public Users ClientAdmins(int clientId)
        {
            ...
            try
            {
                string query = $"select * from dsto_user where isadmin=1 and client_id = '{clientId}' and Deleted=0";
                var table = DbInfo.ExecuteSelectQuery(query);
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
        ...
        public Users ConfigurationUsers(int configuration_Id)
        {
            ...
            try
            {
                string query = $"select dsto_user.* from dsto_user inner join dsto_configurationuser on dsto_user.OID = dsto_configurationuser.yref_user where dsto_configurationuser.yref_configuration='{configuration_Id}' and dsto_user.Deleted=0";
                var table = DbInfo.ExecuteSelectQuery(query);
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
        ...
        public IList<User> GetSuperAdmins()
        {
                    ...
                    if (guid == null)
                    {
                        ...
                        if(user.IsAdmin)
                            innerHtml = "   <h1 style='text-align: center;color: #299def;'>Welcome aboard</h1>"
                                      ...
                                      + "   <p>We’re excited you're joining us! You have been invited by DCAnalytics as a System Admin or owner of the account.<p>"
                                      + "   <p>" + "Please click on the link below to complete your account registration" + "</p>"
                                      + "   <a href='" + $"http://www.DCAnalyticsapp.com/#/registration?Id={oid}" + "'> Click here to proceed </a>"
                                      + "   <h5 style='margin-top: 85px;'> Best Regards,</h5>"
                                      + "   <span> DCAnalytics Team </span>";
                        else
