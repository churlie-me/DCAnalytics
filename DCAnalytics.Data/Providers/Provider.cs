...
namespace DCAnalytics.Data
{
    public abstract class Provider
    {
        ...
        public virtual bool Save(DCAnalyticsObject obj)
        {
            return true;
        }
        ...
        public bool RecordExists(string table, string key)
        {
            var query = $"SELECT * FROM [{table}] WHERE [guid]='{key}'";
            return _dbInfo.ExecuteSelectQuery(query).Rows.Count > 0;
        }

        public bool RecordExists(string table, string column, string key)
        {
